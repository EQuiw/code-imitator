...
class Author:
    ...
    def __init__(self, author: str, learnsetup: classification.LearnSetup.LearnSetup):

        self.author = author
        self.authrows = np.where(learnsetup.data_final_test.authors == self.author)[0][0]
        self.authiid = learnsetup.data_final_test.iids[self.authrows]
        self.true_class =  learnsetup.data_final_test.labels[self.authrows]

        # get all source files for author in training set
        source_authrows = np.where(learnsetup.data_final_train.authors == self.author)[0]
        self.all_authiids_train = learnsetup.data_final_train.iids[source_authrows]
    ...
    def check_is_correct(self, learnsetup: classification.LearnSetup.LearnSetup, verbose: bool = True):

        iscorrect: bool = learnsetup.predict(feature_vec=learnsetup.data_final_test.featurematrix[self.authrows, :])\
                          == self.true_class

        if verbose is True:
            print("Check is correct identified:", self.authiid, " with row:", self.authrows)
            scorepred = learnsetup.predict_proba(feature_vec=learnsetup.data_final_test.featurematrix[self.authrows, :],
