...
bool mycmp(const event &ls, const event &rs) {
  return ((ls.when.first) < (rs.when.first));
}
...
void busyCalculation() {
  ...
  for (int i = 0; i < z; i++) {
    ...
    int ti = bank[i].when.first;
    int tf = bank[i].when.second;
    int timeUsed = tf - ti;
    tl[team] -= timeUsed;
  }
}
...
void segmentGeneration() {
  ...
  for (int i = 0; i < z; i++) {
    ...
    int sumTeam = t1 + t2;
    // printf("(T%d,T%d)\n",t1,t2);
    int ti = bank[thisOne].when.second;
    int tf = bank[nextOne].when.first;
    int timeUsed = tf - ti;
    if (timeUsed < 0)
      timeUsed += 1440;
    if (sumTeam == 1) {
...
int solve() {
  ...
  for (int i = 0; i < ac; i++) {
    event tst;
    tst.owner = 0;
    scanf("%d%d", &tst.when.first, &tst.when.second);
    bank.push_back(tst);
  }
  for (int i = 0; i < aj; i++) {
    event tst;
