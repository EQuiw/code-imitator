...
int interval(ppi a, ppi b) {
  return (b.first.first - a.first.second + 1440 * 2) % 1440;
}

bool touching(ppi a, ppi b) { return a.first.second == b.first.first; }

bool isSame(ppi a, ppi b) { return a.second == b.second; }
  ...
  for (int t = 1; t <= tc; t++) {
    ...
    while (1) {
      ...
      for (int i = 0; i < s.size() - 1; i++) {
        ...
        if (isSame(s[i], s[i + 1]) &&
            interval(s[i], s[i + 1]) <= timeleft[s[i].second]) {
      ...
      if (cl == -1) {
        ...
        // run compress
        timeleft[s[cl].second] -= interval(s[cl], s[cl + 1]);
        // cout << "Compress " << s[cl].ET << " to " << s[cl+1].ST << endl;
        // cout << "Time left " << timeleft[s[cl].WHO] << endl;
        s[cl].first.second = s[cl + 1].first.first;
      }
    }
    int ans = 0;
    for (int i = 0; i < s.size() - 1; i++) {
