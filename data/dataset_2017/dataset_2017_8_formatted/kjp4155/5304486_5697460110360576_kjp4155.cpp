...
#define debug(...)                                                             \
  GET_MACRO(__VA_ARGS__, debug4, debug3, debug2, debug1)(__VA_ARGS__)
...
void solve2() {
  ...
  repp(i, N) { repp(j, M) scanf("%lld", &p[i][j]); }
  ...
  if (N == 1) {
    repp(j, M) {
      if (range[1][j].Fi != -1 && range[1][j].Se != -1)
        ans++;
    }
    printf("%lld\n", ans);
    return;
  ...
  do {
    ...
    for (int j = 1; j <= M; j++) {
      ...
      if (range[1][j].Fi == -1 || range[1][j].Se == -1)
        continue;
      if (range[2][jj].Fi == -1 || range[2][jj].Se == -1)
        continue;
      ll curl = range[1][j].Fi;
      ll curr = range[1][j].Se;
      curl = max(curl, range[2][jj].Fi);
      curr = min(curr, range[2][jj].Se);
      if (curl <= curr)
        cnt++;
    }
    ans = max(cnt, ans);
