...
pair<ll, ll> getans(ll k) {
  ...
  for (int i = 0; i < field.size(); ++i) {
    if (!field[i]) {
      if (min(bestans.X, bestans.Y) < min(lefts[i], rights[i])) {
        bestans = {lefts[i], rights[i]}, ind = i;
      } else {
        if (min(bestans.X, bestans.Y) == min(lefts[i], rights[i]))
          if (max(bestans.X, bestans.Y) < max(lefts[i], rights[i]))
            bestans = {lefts[i], rights[i]}, ind = i;
      }
    }
  }
  ...
  if (k != 1)
    return getans(k - 1);
  return {max(bestans.X, bestans.Y), min(bestans.X, bestans.Y)};
}
