...
int main() {
  ...
  rep(cas, 1, T) {
    cin >> n >> m;
    tot = ans = 0;
    rep(i, 1, n) while (!que[i].empty()) que[i].pop();
    ...
    rep(i, 1, tot) {
      // printf("g[%d] = (%d %d %d)\n", g[i].a, g[i].l, g[i].r);
      rep(j, 1, n) while (!que[j].empty() && que[j].top().r < g[i].l) que[j]
          .pop();
      que[g[i].a].push(g[i]);
      int minvalue = 1 << 29;
      rep(j, 1, n) if (que[j].size() < minvalue) minvalue = que[j].size();
      ans += minvalue;
      rep(j, 1, n) for (int k = 1; k <= minvalue; ++k) que[j].pop();
    }
    printf("Case #%d: %d\n", cas, ans);
  }
  return 0;
