...
inline void solve() {
  ...
  rep(i, r) cin >> board[i];
  vector<string> res(r);
  rep(i, r) res[i] = board[i];
  li last_unknown_row = 0;
  rep(i, r) {
    bool empty = true;
    char cc = '?';
    li last_unknown_col = 0;
    rep(j, c) {
      if (board[i][j] != '?') {
        empty = false;
        cc = board[i][j];
        repp(k, last_unknown_col, j) { res[i][k] = cc; }
        last_unknown_col = j + 1;
      }
    }
    if (!empty) {
      // 右端をなんとかする
      repp(k, last_unknown_col, c) { res[i][k] = cc; }
      // 今までの未確定行をなんとかする
      repp(rr, last_unknown_row, i) { res[rr] = res[i]; }
  ...
  repp(i, last_unknown_row, r) { res[i] = res[last_unknown_row - 1]; }
