...
#define RESIDUAL(x, y)                                                         \
  ((y) == e[(x)].a ? e[(x)].cd - e[(x)].f : e[(x)].cr + e[(x)].f)
  ...
  for (; curflow < flowcap && n[cur].point < (int)n[cur].c.size();
    ...
    if (r[next] == r[cur] + 1 && RESIDUAL(cure, cur) > 0) {
      int tmp = dfs(r, SOURCE, SINK, next,
                    min(flowcap - curflow, RESIDUAL(cure, cur)));
      curflow += tmp;
      if (cur == e[cure].a)
        e[cure].f += tmp;
      else
...
int dinic(int N, int SOURCE, int SINK) {
  ...
  while (bfs(r, N, SOURCE, SINK))
    FLOW += dfs(r, SOURCE, SINK, SOURCE, INF);
  return FLOW;
}
...
char color(int id) {
  if (START_R <= id && id < END_R)
    return 'R';
  if (START_O <= id && id < END_O)
    return 'O';
  if (START_Y <= id && id < END_Y)
    return 'Y';
  if (START_G <= id && id < END_G)
    return 'G';
  if (START_B <= id && id < END_B)
    return 'B';
  if (START_V <= id && id < END_V)
    return 'V';
}
  ...
  for (t = 1; t <= T; t++) {
    ...
    for (i = START_Y; i < END_Y; i++)
      for (j = START_B; j < END_B; j++)
        add_edge(i, N + j, 1, 0);
    for (i = START_Y; i < END_Y; i++)
      for (j = START_V; j < END_V; j++)
        add_edge(i, N + j, 1, 0);
    for (i = START_O; i < END_O; i++)
      for (j = START_B; j < END_B; j++)
        add_edge(i, N + j, 1, 0);
