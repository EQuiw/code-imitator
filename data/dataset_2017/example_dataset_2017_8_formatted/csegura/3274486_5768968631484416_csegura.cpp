...
int main() {
  ...
  for (int t = 0; t < tests; t++) {
    ...
    double solution = 0;
    for (int firstSelected = 0; firstSelected < pancakes.size();
         firstSelected++) {
      long long r = pancakes[firstSelected].first;
      double heightSol = 2 * M_PI * pancakes[firstSelected].first *
                         pancakes[firstSelected].second;
      int selected = 1;
      vector<double> admisibleValues;
      for (int i = firstSelected - 1; i >= 0; i--) {
        admisibleValues.push_back(2 * M_PI * pancakes[i].first *
                                  pancakes[i].second);
      }
    ...
    cout << "Case #" << fixed << setprecision(8) << (t + 1) << ": " << solution
         << endl;
  }
}
