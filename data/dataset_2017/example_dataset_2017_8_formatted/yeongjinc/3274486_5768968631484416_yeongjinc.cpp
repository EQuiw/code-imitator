...
int main() {
  ...
  for (int testNo = 1; testNo <= totalTestNo; ++testNo) {
    ...
    auto rcmp = [](const pair<long double, long double> &a,
                   const pair<long double, long double> &b) {
      return a.first > b.first;
    };
    auto scmp = [](const pair<long double, long double> &a,
                   const pair<long double, long double> &b) {
    ...
    for (int i = 0; i <= N - K; ++i) {
      long double cur = PI * cakes[i].first * cakes[i].first +
                        PI * 2.0L * cakes[i].first * cakes[i].second;
      if (K > 1) {
        vector<pair<long double, long double>> work(cakes.begin() + i + 1,
                                                    cakes.end());
