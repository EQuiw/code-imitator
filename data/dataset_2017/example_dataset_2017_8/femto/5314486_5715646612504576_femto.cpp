...
bool check(int x, int y, char c) {
	for(int k = 0; k < 4; k++) {
		if((k == 0 || k == 2) && c == '|') continue;
		if((k == 1 || k == 3) && c == '-') continue;
		int nx = x + dx[k], ny = y + dy[k];
		while(in(nx, ny)) {
			if(b[ny][nx] == '-' || b[ny][nx] == '|') return true;
			if(b[ny][nx] == '#') break;
			nx += dx[k], ny += dy[k];
		}
	}
	...
	for(int y = 0; y < R; y++) {
		for(int x = 0; x < C; x++) {
			if(b[y][x] == '.') {
				...
				if(p[0].size() == 2 || p[1].size() == 2 || p[0].size() + p[1].size() == 0) {
