...
int main(){
	...
	for (int t = 0; t < tests; t++){
		...
		if ((Activities[0].second == 0) && (Activities.back().second == 0)){
			pq.push(Activities[0].first.first + 60 * 24 - Activities.back().first.second);
		}
		int filled = 0;
		...
		if (pq.size()){
			...
			for (int i = 0; i < Activities.size() - 1; i++){
				if (Activities[i].second != Activities[i + 1].second){
					usedZero += (Activities[i+1].first.first - Activities[i].first.second);
				}
			}
			if (Activities[0].second != Activities.back().second){
				usedZero += (Activities[0].first.first + 60 * 24 - Activities.back().first.second);
			}
			if (usedZero >= 720){
				cout << "Case #" << (t+1) << ": " << changes + pq.size() * 2<< endl;
			} else {
				...
				for (int i = 0; i < Activities.size() - 1; i++){
					if ((Activities[i].second == 1) && (Activities[i + 1].second == 1)){
						pq2.push(Activities[i+1].first.first - Activities[i].first.second);
					}
				}
				if ((Activities[0].second == 1) && (Activities.back().second == 1)){
					pq2.push(Activities[0].first.first + 60 * 24 - Activities.back().first.second);
				}
				while(pq2.size()){
					usedZero += pq2.top();
					pq2.pop();
