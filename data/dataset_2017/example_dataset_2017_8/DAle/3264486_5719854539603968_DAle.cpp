...
bool check(int n, const vector<Item>& v) {
	...
	for (auto i : v) {
		...
		if (ch == '+') {
			if (++d1[x + y] > 1) return false;
			if (++d2[x - y + n - 1] > 1) return false;
		}
		if (ch == 'o') {
			if (++col[y] > 1) return false;
			if (++row[x] > 1) return false;
...
int fd2(int n, int x, int y) {
	return x + n - 1 - y;
}

bool add(vector<string>& table, int n, int x, int y, char ch) {
	if (ch == '+') {
		if (d1[fd1(x, y)] != 0 || d2[fd2(n, x, y)] != 0) return false;
		d1[fd1(x, y)]++;
		d2[fd2(n, x, y)]++;
	} else if (ch == 'x') {
		if (r[x] != 0 || c[y] != 0) return false;
		r[x]++; c[y]++;
	} else if (ch == 'o') {
		if (d1[fd1(x, y)] != 0 || d2[fd2(n, x, y)] != 0 || r[x] != 0 || c[y] != 0) return false;
		r[x]++; c[y]++;
		d1[fd1(x, y)]++;
		d2[fd2(n, x, y)]++;
	}
...
int main() {
	...
	for (int test = 0; test < t; ++test) {
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < n; ++j) {
				if (table[i][j] != 'o') {
					if (table[i][j] == '.') add(table, n, i, j, 'o');
					else {
						char ch = table[i][j];
						remove(table, n, i, j, ch);
						if (!add(table, n, i, j, 'o')) add(table, n, i, j, ch);
					}
				} 
			}
		}
		...
		for (int i = 0; i < n; ++i)
			for (int j = 0; j < n; ++j)
				if (old[i][j] != '.') add(table2, n, i, j, old[i][j]);
		for (int i = 0; i < n; ++i) {
			if (table2[0][i] == '.') add(table2, n, 0, i, '+');
		}
		for (int i = 1; i < n; ++i) {
			add(table2, n, i, i, 'x');
		}
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < n; ++j) {
				if (table2[i][j] != 'o') {
					if (table2[i][j] == '.') add(table2, n, i, j, 'o');
					else {
						char ch = table2[i][j];
						remove(table2, n, i, j, ch);
						if (!add(table2, n, i, j, 'o')) add(table2, n, i, j, ch);
					}
				}
			}
		}
