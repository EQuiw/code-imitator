...
void solve(){
    ...
    int cSize=0;
    int jSize=0;
    for(int i=0; i< Ac; i++) {cin>>cAct[i].a>>cAct[i].b; cSize+=cAct[i].b-cAct[i].a;}
    for(int i=0; i< Aj; i++) {cin>>jAct[i].a>>jAct[i].b; jSize += jAct[i].b-jAct[i].a;}
    ...
    for(int i=1; i< Ac; i++){
        bool bad = false;
        for(int j=0; j< Aj; j++){
            if( jAct[j].a>=cAct[i-1].b && jAct[j].a< cAct[i].a)bad = true;
        }
        if(!bad) connects.push_back(cAct[i].a-cAct[i-1].b);
        else connects.push_back(2000);
    }
