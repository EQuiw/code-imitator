...
int left(int k){
    return ceil(((double)prod[k][pos[k]]/1.1)/need[k]);
}
int right(int k){
    return floor(((double)prod[k][pos[k]]/0.9)/need[k]);
}
    ...
    for(int cas = 0; cas < test; cas++){
        ...
        for(int i = 0; i < n; i++){
            for(int j = 0; j < p; j++){
                int a;
                scanf("%d", &a);
                if((ceil(((double)a/1.1)/need[i]))<= (floor(((double)a/0.9)/need[i])))
                    prod[i].push_back(a);
            }
        }
        for(int i = 0; i < n; i++){
