...
//		}
		...
		if(N==2){
			int i = 0,j = 0;
			while(i<P && have[0][i].l == -1) ++i;
			while(j<P && have[1][j].l == -1) ++j;
			while(i<P && j<P){
//				printf("i=%d j=%d\n",i,j);
				...
				else if(have[1][j].h>=have[0][i].l && have[1][j].l <= have[0][i].l){
					++i;
					++j;
					++ans;
				}
				else if(have[0][i].l>=have[1][j].l && have[0][i].h <= have[1][j].h){
					++i;
					++j;
					++ans;
				}
...
/*		for(int i=1;i<N;++i){
			...
			while(j<P && k<P){
				if(have[i-1][j].l <= have[i][k].l && have[i-1][j].h >= have[i][k].h){
					edge[i-1][j][k] = true;
					++j;
					++k;
				}
