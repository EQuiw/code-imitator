...
void calculate(char **g, int r, int c, set<char> ls, char in[1000], int bounds[1000][4]){
    ...
    for (j=0;j<c;j++){
      if (g[i][j]=='?'){
        for (it=ls.begin();it!=ls.end();it++){
          for (k=0;k<1000;k++) if (in[k]==*it) break;
          temp=k;
          bool breaker=0;
          for (it2=ls.begin();it2!=ls.end();it2++){
            if (*it==*it2) continue;
            for (k=0;k<1000;k++) if (in[k]==*it2) break;
            temp2=k;
          ...
          for (k=min(bounds[temp][0],i);k<=max(bounds[temp][2],i);k++){
            for (l=min(bounds[temp][1],j);l<=max(bounds[temp][3],j);l++){
              if (g[k][l]!='?' && g[k][l]!=*it){
                breaker=1;
                break;
              }
