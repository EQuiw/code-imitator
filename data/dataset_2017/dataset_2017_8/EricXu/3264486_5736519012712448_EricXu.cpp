...
int bfs(char *g, int ars, int size){
  ...
  string current="",goal="";
  for (i=0;i<ars;i++) goal+="+";
  for (i=0;i<ars;i++) current+=g[i];
  ...
  while (!v.empty()){
    ...
    v.pop();
    if (temp==goal) return steps; 
    for (i=0;i<=ars-size;i++){
      for (j=i;j<i+size;j++){
        if (temp[j]=='+') temp[j]='-';
        else temp[j]='+';
      }
      if (vis.find(temp)==vis.end()) v.push(make_pair(temp,steps+1));
      temp=copy;
    }
  }
  return -1;
...
int main(){
  ...
  for (i=0;i<t;i++){
    ...
    for (j=0;j<temp.size();j++) g[j]=temp[j];
    int ans=bfs(g,temp.size(),size);
    if (ans==-1) outfile << "Case #" << i+1 << ": IMPOSSIBLE" << endl;
    else outfile << "Case #" << i+1 << ": " << ans << endl;
  }
  return 0;
}
