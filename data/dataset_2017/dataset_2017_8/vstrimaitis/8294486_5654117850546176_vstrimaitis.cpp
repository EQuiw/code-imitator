...
string solve(int n/*, int r, int o, int y, int g, int b, int v*/, vector<pair<int, char>> chrs)
{
	sort(chrs.begin(), chrs.end(), [](pair<int, char> a, pair<int, char> b) -> bool
	{
	...
	FOR(i, 0, o)
	{
		...
		if (cnt == n)
			break;
		idx = add(idx, 2, n);
		while (!(s[idx] == ' ' && (s[prev(idx, n)] == ' ' || s[prev(idx, n)] == 'B') && (s[next(idx, n)] == ' ' || s[prev(idx, n)] == 'B')))
			idx = next(idx, n);
	}
	FOR(i, 0, g)
	{
		...
		if (cnt == n)
			break;
		idx = add(idx, 2, n);
		while (!(s[idx] == ' ' && (s[prev(idx, n)] == ' ' || s[prev(idx, n)] == 'R') && (s[next(idx, n)] == ' ' || s[prev(idx, n)] == 'R')))
			idx = next(idx, n);
	}
	FOR(i, 0, v)
	{
		...
		if (cnt == n)
			break;
		idx = add(idx, 2, n);
		while (!(s[idx] == ' ' && (s[prev(idx, n)] == ' ' || s[prev(idx,n)] == 'Y') && (s[next(idx, n)] == ' ' || s[next(idx, n)] == 'Y')))
			idx = next(idx, n);
	}
	FOR(i, 0, r)
	{
