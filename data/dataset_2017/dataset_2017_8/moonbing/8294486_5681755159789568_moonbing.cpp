...
double Solve(int src, int dst) {
	...
	while (true) {
		...
		if (current.pos == dst) return current.time;

		for (i = 0; i < list[current.pos].size(); i++) {
			int dst = list[current.pos][i].idx;
			double cost = list[current.pos][i].cost;
			if (current.dis >= cost) {
				que.push({ dst,current.time + cost / current.speed,current.dis - cost,current.speed });
			}
			if (h[current.pos].dis >= cost) {
				que.push({ dst,current.time + cost / h[current.pos].speed,h[current.pos].dis - cost,h[current.pos].speed });
			}
		}
	}
	return 0;
