...
LL T,N,P,req[110],a[110][110],K; range r[110][110];
point p[10000];
...
int main(){
    ...
    for (t=1; t<=T; t++){
        ...
        sort(p+1,p+K+1,[](point A, point B){ if (A.x==B.x) return (A.e<B.e); else return (A.x<B.x); });
        tot=0; int res=0;
        memset(cnt,0,sizeof(cnt));
        for (k=1; k<=K; k++){
            if (p[k].e){
                ...
                if (tot==N){
                    for (i=1; i<=N; i++){
                        ...
                        for (j=1; j<=P; j++)
                            if (act[i][j]){
                                if (pos==-1) pos=j;
                                else if (r[i][j].r<r[i][pos].r) pos=j;
                            }
