...
void dk(lld source)
{
	...
	while(!pq.empty())
	{
		...
		vis[curr.first]=true;
		rep(i,0,a[curr.first].size())
			if(!vis[a[curr.first][i].first] and a[curr.first][i].second+curr.second<dis[a[curr.first][i].first])
				pq.push(make_pair(a[curr.first][i].first,(dis[a[curr.first][i].first]=a[curr.first][i].second+curr.second)));
	}
}
int main()
{
	...
	rep(tt,1,t+1)
	{
		...
		rep(i,1,n+1) rep(j,1,n+1) {
			cin>>mat[i][j];
			if(mat[i][j]!=-1) adj[i].pb({j,mat[i][j]});
		}
		rep(i,1,n+1) add(i);
		cout<<"Case #"<<tt<<": ";
		while(q--)
