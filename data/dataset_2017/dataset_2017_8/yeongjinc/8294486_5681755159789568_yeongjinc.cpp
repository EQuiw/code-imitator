  ...
  for (int i = 0; i < N; ++i) {
    ...
    while ( ! pq.empty()) {
      ...
      for (int j = 0; j < N; ++j) {
        ...
        if (dis[cur.second][j] == -1) continue;

        int nextNo = j;
        long double nextCost = cur.first + (static_cast<long double>(dis[cur.second][j]) / ESs[i].second);
        if (nextCost * ESs[i].second <= static_cast<long double>(ESs[i].first)) {
          if (nextCost < adj[i][j]) {
            adj[i][j] = nextCost;
