...
int main(void) {
  ...
  for (int iCase = 1; iCase <= T; ++iCase) {
    ...
    for (i = 0; i < 6; ++i) {
      int chunk = 0;
      for (j = 0; j <= 1; ++j) chunk += count[(i+j)%6];
      if (chunk > maxChunk) {
	maxChunkStart = i;
	maxChunk = chunk;
      }
    ...
    for (i = 0; i < count[plus2]; ++i) sPlus2[i] = colors[plus2];
    for (i = 0; i < count[plus4]; ++i) sPlus4[maxChunk-1 - i] = colors[plus4];
    ...
    for (i = 0; i < maxChunk; ++i) {
      ...
      if (sPlus2[i]!=' ') result += sPlus2[i];
      if (sPlus4[i]!=' ') result += sPlus4[i];
    }
    
    cout << " " << result << "\n";
    if (result.length() < n) cerr << "****" << iCase << "****\n";
  }
