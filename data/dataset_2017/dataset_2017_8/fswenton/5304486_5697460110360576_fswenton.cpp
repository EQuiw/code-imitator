...
int main(void) {
  ...
  for (int iCase = 1; iCase <= T; ++iCase) {
    ...
    for (i = 0; i < N; ++i)
      for (j = 0; j < P; ++j) {
	cin >> packages[i][j];
	minPackServings[i][j] = (10*packages[i][j]+11*needed[i]-1)/(11*needed[i]);
    ...
    if (N==1) {  // Just see whether you can make something from the packs
      for (i = 0; i < P; ++i)
	if (minPackServings[0][i] <= maxPackServings[0][i] && maxPackServings[0][i] > 0)
	  ++best;
	...
	for (i = 0; i < P; ++i) {
	  if (minPackServings[0][i] <= maxPackServings[0][i] && maxPackServings[0][i] > 0
	      && minPackServings[1][perm[i]] <= maxPackServings[1][perm[i]] && maxPackServings[1][perm[i]] > 0
