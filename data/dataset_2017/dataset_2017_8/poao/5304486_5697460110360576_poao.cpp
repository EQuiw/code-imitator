...
long long getu(long long a) {
	return a*11/10;
}
long long getd(long long a) {
	return (a*9+9)/10;
}
	...
	for(int tc=1; tc<=T; tc++) {
		...
		for(int i=0; i<N; i++) {
			for(int j=0; j<P; j++) {
				...
				/*if(Qu[i][j] >= Qd[i][j]) {
					if(!valid(R[i], Q[i][j], Qu[i][j]) || !valid(R[i], Q[i][j], Qu[i][j])) {
						std::cerr << "invalid at " << R[i] << ", " << Q[i][j] << std::endl;
					}
				}*/
				//std::cout << Q[i][j] << ": " << Qu[i][j] << " " << Qd[i][j] << std::endl;
				//printf("%d: %d %d\n", Q[i][j], Qu[i][j], Qd[i][j]);
			}
		}
		int ans = 0;
		if(N==1) {
			...
			do {
				...
				for(int i=0; i<P; i++) {
					...
					j1 = i;
					j2 = lst[i];
					if(valid(R[0], Q[0][j1], Qu[0][j1]) && valid(R[1], Q[1][j2], Qu[0][j1])) {
						nans++;
					}
					else if(valid(R[0], Q[0][j1], Qd[0][j1]) && valid(R[1], Q[1][j2], Qd[0][j1])) {
	 					nans++;
	 				}
					else if(valid(R[0], Q[0][j1], Qu[1][j2]) && valid(R[1], Q[1][j2], Qu[1][j2])) {
	 					nans++;
	 				}
					else if(valid(R[0], Q[0][j1], Qd[1][j2]) && valid(R[1], Q[1][j2], Qd[1][j2])) {
	 					nans++;
	 				}
				}
