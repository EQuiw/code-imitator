	...
	for(int tc=1; tc<=T; tc++) {
		...
		while(true) {
			...
			for(int ord=0; ord<26; ord++) {
				if(appear[ord]) {
					while(rmin[ord]-1>=1 && empty(rmin[ord]-1, rmin[ord]-1, cmin[ord], cmax[ord])) {
						adj = true;
						rmin[ord]--;
						fill(rmin[ord], rmin[ord], cmin[ord], cmax[ord], ord+'A');
					}
					while(rmax[ord]+1<=R && empty(rmax[ord]+1, rmax[ord]+1, cmin[ord], cmax[ord])) {
						adj = true;
						rmax[ord]++;
			...
			for(int ord=0; ord<26; ord++) {
				if(appear[ord]) {
					while(cmin[ord]-1>=1 && empty(rmin[ord], rmax[ord], cmin[ord]-1, cmin[ord]-1)) {
						adj = true;
						cmin[ord]--;
						fill(rmin[ord], rmax[ord], cmin[ord], cmin[ord], ord+'A');
					}
					while(cmax[ord]+1<=C && empty(rmin[ord], rmax[ord], cmax[ord]+1, cmax[ord]+1)) {
						adj = true;
						cmax[ord]++;
