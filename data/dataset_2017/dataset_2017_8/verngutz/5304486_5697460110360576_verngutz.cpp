        ...
        */
        ...
        while(can_move()) {
            ...
            if(valid) {
                ...
                for(int i = 1; i < N; i++) {
                    greatest_lower_bound = max(greatest_lower_bound, b[i][counters[i]].first);
                    least_upper_bound = min(least_upper_bound, b[i][counters[i]].second);
                }
                // cout << greatest_lower_bound << " " << least_upper_bound << endl;
                bool allgood = true;
                for(int i = 0; i < N; i++) {
                    if(!(greatest_lower_bound <= b[i][counters[i]].second && b[i][counters[i]].first <= least_upper_bound)) {
                        allgood = false;
                    }
                }
                if(allgood) {
