...
int main()
{
	...
	for (int kase=1;kase<=test;kase++)
	{
		...
		if (R>Y+B||Y>R+B||B>R+Y) {printf("Case #%d: IMPOSSIBLE\n",kase);continue;}
		
		if (R>=Y&&Y>=B) {a[1]='R';a[2]='Y';a[3]='B';}
		else if (R<=Y&&Y<=B) {a[1]='B';a[2]='Y';a[3]='R';}
		else if (R>=B&&B>=Y) {a[1]='R';a[2]='B';a[3]='Y';}
		else if (R<=B&&B<=Y) {a[1]='Y';a[2]='B';a[3]='R';}
		else if (B>=R&&R>=Y) {a[1]='B';a[2]='R';a[3]='Y';}
		else if (B<=R&&R<=Y) {a[1]='Y';a[2]='R';a[3]='B';}
		if (R<Y) {t=R;R=Y;Y=t;}
		if (R<B) {t=R;R=B;B=t;} 
		if (Y<B) {t=Y;Y=B;B=t;}
		//YRYR...BRBR....YBR.........R
		...
		int i;
		int l=-1;
		for (i=1;i<=R-B;i++) {s[++l]=a[2];s[++l]=a[1];}
		//printf("%c%c",a[2],a[1]);
		for (i=1;i<=R-Y;i++) {s[++l]=a[3];s[++l]=a[1];}
		//for (i=1;i<=R-B;i++) printf("%c%c",a[3],a[1]);
		for (i=1;i<=Y+B-R;i++) {s[++l]=a[2];s[++l]=a[3];s[++l]=a[1];}
