...
while(t--)
{
	...
	rep(i,0,n-1)rep(j,0,n-1)
	{
		cin>>d[i][j];
		dd[i][j]=d[i][j];
		if(d[i][j]==-1)dd[i][j]=mx;
	}
	rep(i,0,n-1)dd[i][i]=0;
	rep(k,0,n-1)rep(i,0,n-1)rep(j,0,n-1)dd[i][j]=min(dd[i][j],dd[i][k]+dd[k][j]);
	int to[105][105]={0};
	rep(i,0,n-1)
	{
		rep(j,0,n-1)
		{
			if(dd[i][j]>e[i])to[i][j]=0;
			else to[i][j]=1;
		}
	}
	...
	while(q--)
	{
		...
		while(!q.empty())
		{
			...
			ld d1=-q.top().ff;
			q.pop();
			if(d1>dis[at][hor])continue;
			rep(i,0,n-1)
			{
				if(i==at)continue;
				if(d[at][i]==-1)continue;
				if(to[hor][i]==0)continue;
				ld time=d[at][i];
				time/=(ld)s[hor];
