...
int find_base(int x, int y) {
	...
	if (y * 100 > x * l * 110) return -1;
	return l;
}

bool valid(int x, int y, int num) {
	if (x * num * 90 <= y * 100 && y * 100 <= x * num * 110) return true;
	return false;
}
...
bool check(int num) {
	for (int i = 2; i <= n; i++) {
		bool ok = false;
		while (!a[i].empty()) {
			if (a[i].back() * 100 < num * need[i] * 90) a[i].pop_back();
			else if (valid(need[i], a[i].back(), num)) {
				ok = true; a[i].pop_back(); break;
			}
			else {
				return false;
			}
...
signed main() {
	...
	while (T--) {
		...
		for (int i = (int)a[1].size() - 1; i >= 0; i--) {
			...
			if (base == -1) {
				a[1].pop_back(); continue;
			}
			for (int j = base;; j++) {
				if (a[1][i] * 100 < need[1]*j * 90) break;
				if (check(j)) {
					ans++; break;
				}
			}
