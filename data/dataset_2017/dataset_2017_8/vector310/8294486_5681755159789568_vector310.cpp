...
void floyd(){
	rep(k, 1, n)
		rep(i, 1, n) if(d[i][k] != -1)
			rep(j, 1, n) if(d[k][j] != -1)
...
void dp(int x){
	...
	while(!q.empty()){
		...
		//printf("f(%d %d %d) = %f\n", now.s, now.t, now.k, f[now.s][now.t][now.k]);
		if(now.t != now.k){
			if(f[now.s][now.t][now.t] < 0 || f[now.s][now.t][now.t] > f[now.s][now.t][now.k]){
		...
		rep(i, 1, n)if(d[now.t][i] >= 0){
			if(maxd[now.k] >= d[now.k][now.t] + d[now.t][i]){
				if(f[now.s][i][now.k] < 0 || f[now.s][i][now.k] > f[now.s][now.t][now.k] + (double)d[now.t][i] / v[now.k]){
					f[now.s][i][now.k] = f[now.s][now.t][now.k] + (double)d[now.t][i] / v[now.k];
