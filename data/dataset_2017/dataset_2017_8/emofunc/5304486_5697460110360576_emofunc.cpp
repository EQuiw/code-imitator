...
void work()
{
    ...
    while (flag) {
        ...
        for (int i=1;i<=n;i++)
        {
            if (Q[i][pos[i]].x<=cur&&Q[i][pos[i]].y>=cur)continue;
            else f = 0;break;
        }
        if (f) {
            ans++;
...
int main()
{
    ...
    for (int cas=1;cas<=T_T;cas++)
    {
        ...
        for (int i=1;i<=n;i++) for (int j=1;j<=p;j++) scanf("%d",&value[i][j]);
        for (int i=1;i<=n;i++)
        {
            for (int j=1;j<=p;j++)
            {
                ...
                if (tmpy <= tmpx) Q[i][j]=node(tmpy,tmpx);
                else Q[i][j] = node(-1,-1);
            }
            sort(Q[i]+1,Q[i]+1+p);
        }
