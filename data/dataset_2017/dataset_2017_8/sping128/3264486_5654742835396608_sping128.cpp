...
void compute(P2 maxx, LL num_maxx, P2 minn, LL num_minn, LL K) {
    ...
    K -= num_minn;
    map<P2, LL> cnt;
    if (num_maxx && maxx.x) cnt[get_pair(maxx.x)] += num_maxx;
    if (num_maxx && maxx.y) cnt[get_pair(maxx.y)] += num_maxx;
    if (num_minn && minn.x) cnt[get_pair(minn.x)] += num_minn;
    if (num_minn && minn.y) cnt[get_pair(minn.y)] += num_minn;
    P2 n_maxx = P2(-1, -1);
    LL n_num_maxx = 0;
    P2 n_minn = P2(-1, -1);
    LL n_num_minn = 0;
