...
bool valid(int a, int b, int c, int e, int cc){
    ...
    for (int i=c;i<=e;i++) for (int j=a;j<=b;j++){
        if ((ans[i][j]!=('A'+cc))&&(ans[i][j]!='?')) return false;
    }
    return true;
}
int main(){
    ...
    for (int ca=1;ca<=t;ca++){
        ...
        for (int i=1;i<=row;i++) for (int j=1;j<=col;j++) {
            cin>>G[i][j]; ans[i][j]=G[i][j];
        }
        memset(r,0,sizeof r); memset(d,0,sizeof d);
        for (int i=0;i<27;i++) l[i]=100,u[i]=100;
        for (int i=1;i<=row;i++) for (int j=1;j<=col;j++) if (G[i][j]!='?'){
            int x=G[i][j]-'A'; 
            l[x]=min(l[x],j); r[x]=max(r[x],j);
            u[x]=min(u[x],i); d[x]=max(d[x],i);
        }
        for (int i=0;i<26;i++) if (l[i]!=100){
            while (valid(l[i]-1,r[i],u[i],d[i],i)) l[i]--; 
            while (valid(l[i],r[i]+1,u[i],d[i],i)) r[i]++; 
            while (valid(l[i],r[i],u[i]-1,d[i],i)) u[i]--; 
            while (valid(l[i],r[i],u[i],d[i]+1,i)) d[i]++;
        ...
        for (int i=1;i<=row;i++){
            for (int j=1;j<=col;j++) cout<<ans[i][j]; 
            cout<<endl;
        }
    }
    return 0;
