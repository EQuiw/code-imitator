...
void solve() {
    ...
    if(N <= 2) {
        rep(pack, P) {
            ...
            bool gok = false;
            REP(i, from, to+1) {
                bool ok = (need[0][pack].first <= R[0]*i && R[0]*i <= need[0][pack].second);
//                cout << " fc: " << need[0][pack].first << " " << R[0]*i << " & "<<  R[0]*i << " " << need[0][pack].second << endl;
...
//                            cout << need[g][p].first << " " << R[g]*i << " & "<<  R[g]*i << " " << need[g][p].second << endl;
                            ok = true;
                            need[g][p] = mp(-1, -1);
                            break;
                        }
