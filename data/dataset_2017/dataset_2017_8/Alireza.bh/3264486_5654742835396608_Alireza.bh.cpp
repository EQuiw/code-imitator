	...
	FR(cas,Tcas){
		...
		while(freeSlot<k){
			...
			for(map<PLL,ll>::iterator it=mp.begin();it!=mp.end();++it){
				...
				if(temp.find(p1)==temp.end()) temp[p1]=0;
				if(temp.find(p2)==temp.end()) temp[p2]=0;
				temp[p1]+=it->second;
				temp[p2]+=it->second;
			}
			mp=temp;
			if(freeSlot>=k) {k=k-(freeSlot-two);break;}
		}
		for(map<PLL,ll>::iterator it=mp.begin();it!=mp.end();++it)
			res.push_back(*it);
		sort(ALL(res));
		reverse(ALL(res));
		FR(i,res.size()){
			if(k<=res[i].second) {cout<<res[i].first.second<<" "<<res[i].first.first<<endl;break;}
			else k-=res[i].second;
		}
	}
}
