...
int test1(ll nd) {
    ...
    if(dev==0) return 1;
        ...
        if(11*(nd-dev) >= 10*nd) return 1;
        if(10*nd >= 9*(nd-dev+a[0])) return 1;
        return 0;
    }
}
...
//        ll ca = x*a[0];
//        ll cb = x*a[1];
//        if(nda*10 <= ca*11 && nda*10 >= ca*9 && ndb*10 <= cb*11 && ndb*10 >= cb*9) return 1;
//    }
    ll mx = min((10*nda)/(9*a[0]), (10*ndb)/(9*a[1]));
    ll mn = max((10*nda+11*a[0]-1)/(11*a[0]), (10*ndb+11*a[1]-1)/(11*a[1]));
    if(mx>=mn) return 1;
    return 0;
}
