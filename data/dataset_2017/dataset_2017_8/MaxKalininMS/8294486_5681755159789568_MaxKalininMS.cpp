...
std::vector<D> dijkstra(const Graph& g, const std::vector<D>& len,
    ...
    const std::vector<bool>* haveEdge = nullptr, const std::vector<D>* potentials = nullptr,
    std::vector<int>* prevVertex = nullptr, std::vector<int>* prevEdge = nullptr);
...
public:
    ...
    void solve(std::istream& in, std::ostream& out) {
		...
		for (int test = 1; test <= T; ++test) {
			...
			GraphV g(N*N);
			vector<double> edge;
			for (int city = 0; city < N; ++city) for (int horse = 0; horse < N; ++horse) {
				...
				for (int i = 0; i < N; ++i) if (d[city][i] != INF && d[city][i] + curDist <= hdist[horse]) {
