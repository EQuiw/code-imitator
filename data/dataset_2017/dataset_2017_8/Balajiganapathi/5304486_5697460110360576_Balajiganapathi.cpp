...
void init() {
    fo(i, n) fo(j, p) ranges[i][j] = calc(i, j);
}
...
int solve() {
    ...
    for(int mask = (1 << (n * p)) - 1; mask >= 0; --mask) {
        dp[mask] = 0;
        fo(nxt1, n * p) if(!is1(mask, nxt1)) {
            Range r = ranges[nxt1 / p][nxt1 % p];
            if(n == 1) {
                if(r.isvalid()) dp[mask] = max(dp[mask], 1 + dp[mask|(1 << nxt1)]);
            } else {
                fo(nxt2, n * p) if(nxt2 / p != nxt1 / p && !is1(mask, nxt2)) {
                    Range cur = r.intersect(ranges[nxt2 / p][nxt2 % p]);
                    if(cur.isvalid()) dp[mask] = max(dp[mask], 1 + dp[mask|(1 << nxt1)|(1 << nxt2)]);
                }
            }
        }
        //trace(mask, dp[mask]);
