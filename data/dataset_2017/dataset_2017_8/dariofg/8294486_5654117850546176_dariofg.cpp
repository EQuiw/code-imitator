	...
	for(int I=1;I<=T;I++)
	{
		...
		for(int ii=0;ii<100;ii++)
		{
			...
			while(ni>0)
			{
				...
				if(ri)vc.push_back('R');
				if(oi)vc.push_back('O');
				if(yi)vc.push_back('Y');
				if(gi)vc.push_back('G');
				if(bi)vc.push_back('B');
				if(vi)vc.push_back('V');
				random_shuffle(vc.begin(),vc.end());
				if(ni==n)
				{
					ni--;
				...
				for(int i=0;i<aaa.size();i++)
				{
					if(aaa[i].first<0 && aaa[i].second=='R' && comp(ans[ans.size()-1],'R'))
					{
						ans.push_back('R');
						ni--;
						ri--;
					...
					if(aaa[i].first<0 && aaa[i].second=='O' && comp(ans[ans.size()-1],'O'))
					{
						ans.push_back('O');
						ni--;
						oi--;
					...
					if(aaa[i].first<0 && aaa[i].second=='Y' && comp(ans[ans.size()-1],'Y'))
					{
						ans.push_back('Y');
						ni--;
						yi--;
					...
					if(aaa[i].first<0 && aaa[i].second=='G' && comp(ans[ans.size()-1],'G'))
					{
						ans.push_back('G');
						ni--;
						gi--;
					...
					if(aaa[i].first<0 && aaa[i].second=='B' && comp(ans[ans.size()-1],'B'))
					{
						ans.push_back('B');
						ni--;
						bi--;
					...
					if(aaa[i].first<0 && aaa[i].second=='V' && comp(ans[ans.size()-1],'V'))
					{
						ans.push_back('V');
						ni--;
						yi--;
			...
			if(!comp(ans[0],ans[ans.size()-1]))ni++;
			if(!ni)
			{
				for(int i=0;i<n;i++)
					cout << ans[i];
