...
int count(vector<vector<bool> > & ok, vector<vector<int> > & izs,vector<vector<int> > & ders)
{
	int n = izs.size();
	int p = izs[0].size();
	if(n==1)
	...
	else // n==2
	{
		...
		for(int i=0;i<p;i++)
		{
			for(int j=0;j<p;j++)
			{
				if(ok[0][i] && ok[1][j] && (izs[1][j]<=ders[0][i] && izs[0][i]<=ders[1][j]))
				{
					pp.first = i;
					pp.second = j;
					vp.push_back(pp);
	...
	for(int I=1;I<=T;I++)
	{
		...
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<p;j++)
			{
				...
				izs[i][j] = (m[i][j]*10)/(a[i]*11)+!!((m[i][j]*10)%(a[i]*11));
