...
int dfs(int k){
	...
	while(1){
		if(tmin[k]<=tmax[k]) 
			re(j,last[k+1]+1,p-1)
				if(minq[k+1][j]<=maxq[k+1][j] && !(minq[k+1][j]>tmax[k] || maxq[k+1][j]<tmin[k]))//satisfy
				{
					flag[k+1] = j;
					if(dfs(k+1)) {mark=1;break;}
				}
		flag[k]++;
		if(flag[k]==p) break;
		if(k==0){
	...
	re(casen, 1, T){
		...
		re(i,0,n-1)
			re(j,0,p-1)
			{
				...
				while((minq[i][j]-1)*r[i]*9<=q[i][j]*10 && (minq[i][j]-1)*r[i]*11>=q[i][j]*10) minq[i][j]--;
				maxq[i][j] = int(float(q[i][j])*10/float(r[i])/9);
				while((maxq[i][j]+1)*r[i]*9<=q[i][j]*10 && (maxq[i][j]+1)*r[i]*11>=q[i][j]*10) maxq[i][j]++;
			}
		ans = 0;
